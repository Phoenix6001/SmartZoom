...
| `FrameMs` | 8 | `Interop/Input/TouchPinchInjector.cs` | Feeds a 120 Hz display and halves the step on 60 Hz |
| `PanDurationMs` / `PanSettleMs` | 140 / 60 | same | The settle stops the browser turning the drag into a fling |
| `ScrollbarAllowance` | 56 | `Core/Zoom/BrowserAdapter.cs` | Classic scrollbars are 17 px at 100% scaling and 51 px at 300%; contacts stay out of that strip |
...
Timing note: `Thread.Sleep` alone quantises to 15.6 ms on Windows, which is what made an early version of the
gesture look steppy. The frame pacer sleeps only above 2 ms and spins below it.
...
`tools/SmartZoom.Probe` is the tool these numbers came out of. It drives the same code the tray app does, so
what it reports is what SmartZoom would do:

```powershell
dotnet run --project tools/SmartZoom.Probe -- help
```

The usual shape of a measurement is: take a screenshot, do one thing, take another, and compare.

```powershell
$probe = "dotnet run --project tools/SmartZoom.Probe --"

# What a press at (2500, 900) would find, and where a x1.9 pinch would put its fingers.
smartzoom-probe windows 2500 900
smartzoom-probe hittest 2500 900
smartzoom-probe plan    2500 900 1.9

# What the recognizer actually delivers for a requested 1.9x.
smartzoom-probe shot  before.png 2500 900
smartzoom-probe pinch 2500 900 1.9
smartzoom-probe shot  after.png 2500 900
smartzoom-probe scale before.png after.png 200 1200 1.0 2.5

# Whether a restore was exact. 0.0% is what a browser should give.
smartzoom-probe diff before.png restored.png
```
